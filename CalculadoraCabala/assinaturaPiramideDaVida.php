...
function assinaturaTrianguloDaVida($nomeCompleto)
{
    ...
    // Converter o nome completo em números usando a tabela e considerando símbolos especiais
    $numeros = [];
    ...
    for ($i = 0; $i < count($nomeArray); $i++) {
        $letra = $nomeArray[$i];
        $valorLetra = isset($tabela[$letra]) ? $tabela[$letra] : 0;
        ...
        // Atualizar os números se o valor da letra for válido
        if ($valorLetra > 0) {
            $numeros[] = $valorLetra;
        }
    }
...
// Exemplo de uso
$nomeCompleto = "Årika Maria Nascimento da Silva Cardoso";
echo assinaturaTrianguloDaVida($nomeCompleto);
